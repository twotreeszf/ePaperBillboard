...
- **TTStreamImage**: LVGL-compatible stream PNG widget (libspng + zlib, vendored in `lib/spng` and `lib/zlib`); decode to screen with I1 passthrough, no cache. Icons live in `data/icons/` (Lucide slices: `wifi.png`, `watch.png`, `clock.png`, `clock_sm.png`, `settings.png`, `back.png`).
